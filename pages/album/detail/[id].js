...
export default function Album({ id }) {
  ...
  if (error) {
    return (
      <Layout title="Album Detail - MyMovie">
  ...
  return (
    <Layout title={`${data ? data[0]?.name + " - MyMovie" : 'Album Detail - MyMovie'}`}>
            ...
            {data[0]?.songs?.map((item, index) => {
              return (
                <TableSimple.tr key={index}>
                  ...
                  <TableSimple.td>
                    <Link href={`/song/detail/${item.id}`} className="text-emerald-500 hover:text-emerald-600 text-sm font-medium focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-emerald-500 rounded">
                      {item.name}
                    </Link>
                  </TableSimple.td>
                  <TableSimple.td>
