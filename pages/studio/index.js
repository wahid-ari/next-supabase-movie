...
export default function Studio() {
  ...
  const [createItem, setCreateItem] = useState({ name: "", image_url: "", city: "", country_id: null })
  const [editItem, setEditItem] = useState({ name: "", image_url: "", city: "", country_id: null })
  const [deleteItem, setDeleteItem] = useState({ name: "", image_url: "", city: "", country_id: null })
  ...
  async function handleEdit() {
    ...
    try {
      const res = await axios.put(`${process.env.API_ROUTE}/api/studio`, editItem)
      if (res.status == 201) {
  ...
  function handleShowEditModal(id, name, image_url, city, country_id) {
    setEditItem({ id: id, name: name, image_url: image_url, city: city, country_id: country_id })
    ...
    <Layout title="Studio - MyMovie">
      ...
      <div className="flex flex-wrap justify-between items-center mb-6 gap-y-3">
        ...
        <Button.success onClick={() => setOpenCreateDialog(true)} className="flex gap-2 items-center">
          <PlusSmIcon className="h-5 w-5" />Add Studio
        </Button.success>
      </div>
      ...
      >
        <div className="mt-5">
          ...
          />
          <LabeledInput label="City (Optional)" type="text" name="city"
              ...
              <TableSimple.tr key={index}>
                ...
                <TableSimple.td>
                  <Button className="!py-[2px] !px-[6px] mr-2"
                    onClick={() => handleShowEditModal(item.id, item.name, item.image_url, item.city, item.countries?.id)}>
                    Edit
                  </Button>
