    ...
    <Layout title="Artist - MyMovie">
      ...
      <div className="flex flex-wrap justify-between items-center mb-6 gap-y-3">
        ...
        <Button.success onClick={() => setOpenCreateDialog(true)} className="flex gap-2 items-center">
          <PlusSmIcon className="h-5 w-5" />Add Artist
        </Button.success>
      </div>
      ...
      >
        <div className="mt-5">
          ...
          />
          <Select label="Genre" name="genre" className="h-10"
            value={createItem.genre_id}
            onChange={(e) =>
              setCreateItem({ ...createItem, genre_id: e.target.value })
            }
          >
            <Select.option className="hidden" value={null}>Select Genre</Select.option>
            {genre?.map(item => (
      ...
      >
        <div className="mt-5">
          ...
          <Select label="Genre" name="genre" className="h-10"
            ...
            onChange={(e) =>
              setEditItem({ ...editItem, genre_id: e.target.value })
            }
          >
            <Select.option className="hidden" value={null}>Select Genre</Select.option>
            {genre?.map(item => (
          ...
          {data.map((item, index) => {
            return (
              <TableSimple.tr key={index}>
                ...
                <TableSimple.td>
                  <Link href={`genre/detail/${item.genre?.id}`} className="text-emerald-500 hover:text-emerald-600 text-sm font-medium focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-emerald-500 rounded">
                    {item.genre?.name}
                  </Link>
                </TableSimple.td>
