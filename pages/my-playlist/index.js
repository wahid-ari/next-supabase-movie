...
export default function Playlist({ id }) {
  const { data, error } = useSWR(`${process.env.API_ROUTE}/api/my-playlist?id=${id}`, fetcher)
  ...
  async function handleCreate() {
    ...
    try {
      const res = await axios.post(`${process.env.API_ROUTE}/api/my-playlist`,
      ...
      mutate(`${process.env.API_ROUTE}/api/my-playlist?id=${id}`)
    }
  }
  ...
  async function handleEdit() {
    ...
    try {
      const res = await axios.put(`${process.env.API_ROUTE}/api/my-playlist`, editItem)
      if (res.status == 201) {
  ...
  async function handleDelete() {
    ...
    try {
      const res = await axios.delete(`${process.env.API_ROUTE}/api/my-playlist?id=${deleteItem.id}`)
      if (res.status == 200) {
  ...
  if (error) {
    return (
      <Layout title="My Playlist - MyMovie">
          ...
          {data.map((item, index) => {
            return (
              <TableSimple.tr key={index}>
                ...
                <TableSimple.td>
                  <Link href={`my-playlist/detail/${item.id}`} className="text-emerald-500 hover:text-emerald-600 text-sm font-medium focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-emerald-500 rounded">
                    {item.name}
                  </Link>
                </TableSimple.td>
