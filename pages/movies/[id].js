...
export async function getServerSideProps(context) {
  ...
  const res = await fetch(`${process.env.API_ROUTE}/api/movie?id=${id}`).then((res) => res.json());
  return {
    props: {
      id: id,
      fallback: {
...
function Page({ id }) {
  ...
  return (
    ...
    >
            ...
            <div className='sm:w-2/3 md:w-3/4 xl:w-3/4 2xl:w-4/5'>
              ...
              {data.actors.length > 0 ?
                <div className="flex flex-wrap gap-2">
                  {data.actors?.map((actor, index) =>
                    <Link key={actor.id}
                      href={`/actor/${actor.id}`}
                      className="truncate text-base text-sky-500 hover:text-sky-600 transition-all duration-300"
                    >
              ...
              {data.directors?.id ?
                <Link
                  ...
                  className="text-base text-sky-500 hover:text-sky-600 transition-all duration-300"
                >
                  {data.directors?.name}
                </Link>
                :
      ...
      {data ? (
        <>
          <Heading h2 className='mt-8 mb-3 !text-[19px]'>
            Actors
          </Heading>
          <div
            className={`${
            ...
            {data.actors.length > 0 ? (
              data.actors?.map((actor, index) => (
                ...
                >
                  ...
                  <div className='p-2'>
                    <p className='overflow-hidden text-ellipsis text-[15px] transition-all duration-300 group-hover:text-sky-500'>
                      {actor.name}
                    </p>
                  </div>
                </Link>
      ...
      {data ? (
        <>
          <Heading h2 className='mt-3 mb-3 !text-[19px]'>
            Director
          </Heading>
