...
export default function Actor({ id }) {
  ...
  useEffect(() => {
    if (data) {
      ...
      setSelectedCountry({ id: data[0].countries?.id, name: data[0].countries?.name })
    }
  }, [data])
  ...
  return (
    <Layout title={`Edit ${data ? data[0]?.name + " - MyMovie" : 'Edit Actor - MyMovie'}`}>
      ...
      {data ?
        <div className="max-w-lg rounded">
          ...
          <LabeledInput label="Instagram URL (Optional)" type="text" name="instagram"
          ...
          <LabeledInput label="Twitter URL (Optional)" type="text" name="twitter"
